...
def _ensure_checker_utils(project_dir: Path) -> None:
    """
    Copy project_gen/templates/checkers/utils.py into the project's checkers/
    directory the first time fill_skipped writes a checker.
    """
    dest = project_dir / "checkers" / "utils.py"
    if dest.exists():
        return
    dest.parent.mkdir(parents=True, exist_ok=True)
    src = Path(__file__).parent.parent / "templates" / "checkers" / "utils.py"
